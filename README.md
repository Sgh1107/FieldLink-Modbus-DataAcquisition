...
- ✅ **智能断线处理**：对端主动断开/网络中断时弹窗询问"是否重连"——选「是」立即重连并恢复自动重连循环，选「否」等待手动连接；用户主动断开不弹窗，连接失败不误报
...
- ✅ **MQTT 数据上送**：内置零依赖 MQTT 3.1.1 发布端（**QoS 0/1**，QoS1 含 PUBACK 确认 + 超时 DUP 重发），采集数据/报警事件/连接状态实时上送 broker，自动重连 + 心跳保活，断连期间消息静默丢弃不刷屏，无缝对接 IoT 平台与组态软件
...
│   │
│   ├── ── 可靠性与安全 ──
│   ├── reliabilitymanager.h  # 自动重连/心跳/连续失败告警（对端断开弹窗询问在主窗口处理）
│   ├── securitymanager.h     # 用户/角色/权限、加盐哈希、Token 鉴权
│   ├── credentialcodec.h     # 凭据混淆编解码（MQTT broker 密码不明文落盘）
