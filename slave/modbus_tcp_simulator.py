...
【使用方式】
  方式一（推荐）：FieldLink 程序内菜单 Advanced → Simulator (Modbus Slave) 一键启停，
                 无需手动执行本脚本（面板会自动带上 --port/--unit 参数）。
  方式二（命令行）：
    python slave/modbus_tcp_simulator.py                     # 监听 0.0.0.0:1502
    python slave/modbus_tcp_simulator.py --port 502          # 标准 Modbus TCP 端口
    python slave/modbus_tcp_simulator.py --unit 1            # 指定从站地址(默认1)
    python slave/modbus_tcp_simulator.py --selftest          # 内置 9 项协议自检（不需要 FieldLink）

【连接方法】
  FieldLink 主界面连接类型选 TCP，地址填 127.0.0.1:<端口>，从站地址填 <从站地址>，
  连接后即可读取下方寄存器数据（配合轮询/报警/MQTT 走通全链路）。
...
支持功能码：FC01/02/03/04/05/06/15/16，非法功能/越界地址返回标准异常码。
"""
